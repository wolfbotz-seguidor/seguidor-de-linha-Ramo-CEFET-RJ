...
double Kp = 0.074, Kd = 0.32, Ki = 0.000015; // Variáveis que são modificadas no PID
int PWM = 100; // valor da força do motor em linha reta
...
void setup() {
  ...
  Serial.print(borda.calibratedMinimumOn[0]);
  Serial.println(borda.calibratedMaximumOn[0]);

  //----> Calibração dos Sensores frontais <----\\

  for (int i = 0; i < 100; i++)
    {
      position_line = qtra.readLine(sensor_values);
      
      if (position_line > 4900) {
        digitalWrite(AIN1, HIGH);
        digitalWrite(AIN2, LOW);
        digitalWrite(BIN2, LOW);
        digitalWrite(BIN1, HIGH);
